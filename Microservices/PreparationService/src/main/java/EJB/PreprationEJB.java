...
public class PreprationEJB implements PreprationEJBLocal {
    ...
    public PHResponseType sendOrderToDelivery(String orderid, String outletid) {
        ...
        try (KafkaConnection con = kafka.createConnection()) {
            ...
            dto.setOutledtid(outletid);
            String payload = KafkaUtil.phResponseTypeToJson(dto);
            con.send(new ProducerRecord("prepare-request", correlationId, payload));
            if(holder.getPreparationResponseKey().getNow(null)!=null){
                holder.reset();
            }
            //Below code for api
            //Response res = client.deliveryPersonAllocation(orderid, outletid);
            if (holder.getPreparationResponseKey().get().equals(correlationId)) {
                response = KafkaUtil.jsonToPhResponseType(holder.getPreparationResponseValue().get());
                return response;
            }
            else{
                throw new Exception("Correlation ID Unmatched: "+ correlationId + holder.getPreparationResponseKey().get());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
